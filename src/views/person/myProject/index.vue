    ...
    <div class="person_content">
      ...
      <div class="project-menu">
        <div v-for="(item , index) in pageList" :key="index" class="project-item clearfix">
          ...
          <span class="deliver" v-if="item.status == '10'">约谈：
            <a>0</a>
          </span>
          ...
          <span class="flr" v-if="item.status != '0'" :class="item.status  == '5' ? ' being':'' + item.status == '10' ? ' already':'' + item.status == '15'? ' fail':''">{{item.status == '5' ? '审核中':'' + item.status == '10' ? '已发布':'' + item.status == '15'? '':''}}</span>
...
<style scoped lang="scss">
  ...
  .project-item {
    ...
    .project-item-content {
      position: relative;
      width: 620px;
      line-height: 2; // max-height: 100px;
      overflow: hidden;
    }
